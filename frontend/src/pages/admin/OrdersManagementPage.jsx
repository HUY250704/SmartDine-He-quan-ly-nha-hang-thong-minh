        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-6">
          {filtered.map((order) => {
            ...
            const total = order.items
              ? order.items.reduce((s, it) => s + (normalizeVND(it.menuItemId?.price || it.menuItem?.price || 0) * it.quantity), 0)
              : 0;
                    ...
                    return (
                      <div key={j} className={`flex justify-between items-start ${isServed ? "opacity-50" : ""}`}>
                        ...
                        <span className="text-on-surface font-mono text-xs ml-2">
                          {formatPrice(normalizeVND(item.menuItemId?.price || item.price || 0) * item.quantity)}
                        </span>
                      </div>
                    );
