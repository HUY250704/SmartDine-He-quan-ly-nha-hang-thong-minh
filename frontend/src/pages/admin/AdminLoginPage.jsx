...
export default function AdminLoginPage() {
  ...
  // Nếu đã xác thực xong và đã đăng nhập → redirect
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // login() set token + user trong AuthContext
      // React sẽ re-render và isAuthenticated=true → Navigate sẽ kích hoạt
    } catch (err) {
      const msg =
  ...
  // Loading spinner khi đang kiểm tra token
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "radial-gradient(ellipse at 30% 20%, #1a2333 0%, #0c1322 70%)" }}>
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-on-surface-variant/60 text-sm">Verifying session...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center"
            ...
            <span className="material-symbols-outlined text-4xl" style={{ color: "#ffc174", fontVariationSettings: "'FILL' 1" }}>
              admin_panel_settings
            </span>
          </div>
          <h1 className="text-[48px] font-bold tracking-[-0.02em] text-white leading-tight">SmartDine</h1>
          <p className="text-on-surface-variant/60 text-xs uppercase tracking-[0.3em] mt-2">Admin Control Panel</p>
        </div>
        ...
        >
          ...
          <div
            ...
            style={{
              background: "radial-gradient(circle 200px at var(--mouse-x,50%) var(--mouse-y,50%), rgba(255,193,116,0.15), transparent 80%)",
            }}
          />
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h2 className="text-white font-bold text-[24px]">Authentication</h2>
                <p className="text-on-surface-variant/50 text-xs">Secure access required</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Username */}
              <div>
                <label className="block text-on-surface-variant/80 text-[11px] font-semibold uppercase tracking-wider mb-2">
                  Operator ID
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    style={{
                      ...
                      border: username.length > 3 ? "1px solid rgba(48,200,143,0.4)" : "1px solid rgba(255,255,255,0.1)",
                    }}
                  />
                  {username.length > 3 && (
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-11 pr-12 py-3 rounded-xl text-sm text-on-surface placeholder-on-surface-variant/30 outline-none transition-all duration-300 ${
                      error ? "animate-shake" : ""
                    }`}
                    style={{
                      background: "rgba(255,255,255,0.05)",
                      backdropFilter: "blur(8px)",
                      border: error ? "1px solid rgba(255,180,171,0.5)" : "1px solid rgba(255,255,255,0.1)",
                    }}
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant/40 hover:text-white/80 transition-colors"
                  >
              ...
              >
                ...
                <span>{submitting ? "Securing Connection..." : "Authenticate"}</span>
              </button>
            </form>
            ...
            <div className="mt-7 pt-7 border-t flex flex-col items-center space-y-4" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
              ...
              <div className="font-mono text-[10px] text-white/20 tracking-[0.2em] uppercase">
                Encrypted Session ID: 77-SD-4091-LX
              </div>
            </div>
          </div>
        </div>
