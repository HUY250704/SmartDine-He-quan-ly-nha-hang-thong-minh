      ...
      <div className="rounded-2xl p-5 md:p-6 mb-6 bill-print-area" style={glassCard}>
        ...
        <div className="space-y-2 mb-4">
          {(bill.items || []).map((item, i) => (
            <div key={i} className="flex justify-between items-start text-xs md:text-sm">
              ...
              <span className="text-on-surface font-mono text-xs ml-2 shrink-0">{formatPrice(normalizeVND(item.price) * (item.quantity || 1))}</span>
            </div>
          ))}
        </div>
        ...
        <div className="border-t border-white/10 pt-4 space-y-1.5 text-xs md:text-sm">
          <div className="flex justify-between">
            ...
            <span className="text-on-surface font-mono">{formatPrice(normalizeVND(bill.subtotal))}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-on-surface-variant/50">Tax (8%)</span>
            <span className="text-on-surface font-mono">{formatPrice(normalizeVND(bill.tax))}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-on-surface-variant/50">Service (5%)</span>
            <span className="text-on-surface font-mono">{formatPrice(normalizeVND(bill.serviceCharge))}</span>
          </div>
          <div className="flex justify-between pt-2 border-t border-white/10">
            <span className="text-white font-bold">Total</span>
            <span className="font-mono font-bold text-base md:text-lg" style={{ color: "#ffc174" }}>
              {formatPrice(normalizeVND(bill.total))}
            </span>
          </div>
        </div>
