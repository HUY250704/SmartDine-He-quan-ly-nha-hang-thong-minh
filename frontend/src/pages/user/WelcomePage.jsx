    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden" style={{ background: "radial-gradient(circle at center, #191f2f 0%, #0c1322 100%)" }}>
      ...
      <div className="w-full max-w-lg relative z-10">
        ...
        <div className="text-center mb-10 animate-floating">
          ...
          <p className="text-on-surface-variant text-lg">Fine Dining, Effortlessly Managed</p>
        ...
        <div className="rounded-3xl p-6 space-y-4" style={{ backdropFilter: "blur(20px)", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 8px 32px 0 rgba(0,0,0,0.37)" }}>
          <div className="text-center pb-4 border-b border-white/5">
            <p className="text-on-surface-variant text-sm">Welcome, </p>
            <p className="font-mono text-2xl font-bold mt-1" style={{ color: "#ffc174" }}>Table {tableId}</p>
          </div>

          {[{ to: `/customer/${tableId}/menu`, icon: "restaurant_menu", iconBg: "bg-primary/20", iconHover: "group-hover:bg-primary/40", iconColor: "text-primary", title: "View Menu", desc: "Explore our curated flavors" },
            { to: `/customer/${tableId}/support`, icon: "support_agent", iconBg: "bg-tertiary/10", iconHover: "group-hover:bg-tertiary/30", iconColor: "text-tertiary", title: "Call Staff", desc: "Our team is at your service" },
            { to: `/customer/${tableId}/tracking`, icon: "receipt_long", iconBg: "bg-secondary/10", iconHover: "group-hover:bg-secondary/30", iconColor: "text-secondary", title: "View My Orders", desc: "Track your dining status" }].map((item, i) => (
            <button key={i} onClick={() => navigate(item.to)}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <span className="block font-semibold text-white text-lg">{item.title}</span>
